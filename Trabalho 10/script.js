        ...
        for (var i = 0; i < agents.length; i++) {
            for (var j = i + 1; j < agents.length; j++) {
                ...
                if (dist < this.PHYSICAL_RADIUS * 2) {
                    ...
                    // APLICA O EFEITO DE COLISÃO
                    a1.triggerCollisionEffect();
                    a2.triggerCollisionEffect();
                }
...
var BaseAgent = /** @class */ (function () {
    function BaseAgent(path, color) {
        ...
        // Variáveis de controle de confusão
        this.confusionSteps = 0;
        this.path = path;
        this.pos = path[0];
        this.startPos = path[0];
        // O último ponto do path é o destino final
        this.finalDest = path[path.length - 1];
        this.step = 0;
        this.color = color;
        this.movementAdapter = new RectangularAdapter();
    }
    ...
    BaseAgent.prototype.notify = function () { for (var _i = 0, _a = this.observers; _i < _a.length; _i++) {
        var observer = _a[_i];
    ...
    BaseAgent.prototype.respawn = function () {
        ...
        this.confusionSteps = 0;
        this.path = pathfinder.findPath(this.startPos, this.finalDest);
    };
    ...
    BaseAgent.prototype.setCollisionState = function (state) { this.collisionState = state; };
    BaseAgent.prototype.getCollisionState = function () { return this.collisionState; };
    // Implementação do efeito de colisão
    BaseAgent.prototype.triggerCollisionEffect = function () {
        // Se já está confuso, não faz nada (ou poderia aumentar o tempo)
        if (this.confusionSteps > 0)
            return;
        // Fica confuso por 5 a 15 frames (tempo aleatório)
        this.confusionSteps = Math.floor(Math.random() * 10) + 5;
        console.log("Agent colidiu! Confuso por ".concat(this.confusionSteps, " steps."));
    };
    BaseAgent.prototype.updateLogic = function () {
        if (this.life <= 0)
            return;
        // Lógica de colisão/Confusão
        if (this.confusionSteps > 0) {
            this.confusionSteps--;
            // Atraso de velocidade: 50% de chance de NÃO se mover neste frame (simula velocidade variável)
            if (Math.random() > 0.5)
                return;
            // Movimento Aleatório (Direção Aleatória)
            // Usa o Adapter para pegar células vizinhas livres
            var neighbors = this.movementAdapter.getNeighbors(this.pos);
            if (neighbors.length > 0) {
                var randomNeighbor = neighbors[Math.floor(Math.random() * neighbors.length)];
                // Executa movimento
                var moveCmd = new MoveAgentCommand(this, this.pos, randomNeighbor);
                commandInvoker.executeCommand(moveCmd);
            }
            // Se a confusão acabou AGORA, precisamos recalcular o caminho até o destino
            if (this.confusionSteps === 0) {
                // Recalcula rota da posição atual até o destino original
                // Nota: Usamos a instância global 'pathfinder' definida no arquivo principal
                this.path = pathfinder.findPath(this.pos, this.finalDest);
                this.step = 0; // Reinicia o step pois é um novo path começando do index 0
            }
            return;
        }
        // Lógica Normal de Caminho
    ...
    BaseAgent.prototype.draw = function (ctx) {
        ...
        // Desenha visual diferente se estiver confuso
        if (this.confusionSteps > 0) {
            ctx.fillStyle = "purple"; // Cor de confusão
            ctx.fillText("?", cx - 5, cy - 5);
        }
        else if (this.collisionState === CollisionState.Colliding) {
            ctx.fillStyle = "red";
        }
        else {
...
var AgentDecorator = /** @class */ (function () {
    ...
    AgentDecorator.prototype.triggerCollisionEffect = function () { this.wrappedAgent.triggerCollisionEffect(); };
