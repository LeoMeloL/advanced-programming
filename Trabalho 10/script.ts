...
class CollisionSystem {
    private readonly PHYSICAL_RADIUS = 0.5;
    private readonly WARNING_RADIUS = 1.5;

    public checkCollisions(agents: IAgent[]): void {
        ...
        for (let i = 0; i < agents.length; i++) {
            for (let j = i + 1; j < agents.length; j++) {
                ...
                const dist = this.getDistance(a1.getPos(), a2.getPos());
...
class BaseAgent implements IAgent {
    ...
    private finalDest: Point;
    private collisionState: CollisionState = CollisionState.None;

    private confusionSteps: number = 0;
    private movementAdapter: IGridAdapter; 
    ...
    notify(): void { for (const observer of this.observers) observer.update(this); }
    ...
    setCollisionState(state: CollisionState): void { this.collisionState = state; }
    getCollisionState(): CollisionState { return this.collisionState; }

    triggerCollisionEffect(): void {
        if (this.confusionSteps > 0) return;

        this.confusionSteps = Math.floor(Math.random() * 10) + 5;
        console.log(`Agent colidiu, Confuso por ${this.confusionSteps} steps.`);
    }
    ...
    updateLogic(): void {
        ...
        if (this.confusionSteps > 0) {
            ...
            const neighbors = this.movementAdapter.getNeighbors(this.pos);
            if (neighbors.length > 0) {
                const randomNeighbor = neighbors[Math.floor(Math.random() * neighbors.length)];
                
                // Executa movimento
                const moveCmd = new MoveAgentCommand(this, this.pos, randomNeighbor);
...
abstract class AgentDecorator implements IAgent {
    ...
    setCollisionState(state: CollisionState) { this.wrappedAgent.setCollisionState(state); }
    getCollisionState() { return this.wrappedAgent.getCollisionState(); }
    triggerCollisionEffect() { this.wrappedAgent.triggerCollisionEffect(); }
}
